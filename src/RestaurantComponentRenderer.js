...
export const components = {
  ...
  innerPages: {
    ...
    OrdersPage: {
      ...
      scrollAnimationDisabled: true,
      url: "/components/innerPages/OrdersPage",
    },
    CheckoutPage: {
      component: CheckoutPage,
      scrollAnimationDisabled: true,
      url: "/components/innerPages/CheckoutPage",
    },
    SignupPage: {
...
export default (props) => {
  const { type, subtype, name } = props;

  const [reloadPage, setReloadPage] = useState(true);

  const [cartMenuCountLocalStorage, setCartMenuCountLocalStorage] =
    useState("");
  const [
    incompleteOrdersCountLocalStorage,
    setIncompleteOrdersCountLocalStorage,
  ] = useState("");
  const [numberOfMenus, setNumberOfMenus] = useState("");
  const fetchIncompleteOrders = async () => {
    const customerDetails = verifyToken();
    try {
      await getAllOrderAvailableAndInCompleteApi({
  ...
  useEffect(() => {
    let cartMenuCountLocalStorage = getLocalStorageProperty(
      "flavors-customer-cart-menu-count"
    );
    let incompleteOrdersCountLocalStorage = getLocalStorageProperty(
    ...
    setCartMenuCountLocalStorage(cartMenuCountLocalStorage);
    setIncompleteOrdersCountLocalStorage(incompleteOrdersCountLocalStorage);
  }, [reloadPage]);
    ...
    if (Component)
      return (
        <>
          <AnimationRevealPage disabled>
            <Header
              cartMenuCountLocalStorage={cartMenuCountLocalStorage}
              incompleteOrdersCountLocalStorage={
                incompleteOrdersCountLocalStorage
              }
            />
